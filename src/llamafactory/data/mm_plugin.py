...
class Qwen2vlPlugin(BasePlugin):
    ...
    def process_messages(
        ...
        for message in messages:
            content = message["content"]
            flags = 0
            if message["role"] == "user" and len(images) > 0:
                if IMAGE_PLACEHOLDER not in content and "Image" in content:
                    flags = 1
                    content = content.replace("Image", "Image:" + IMAGE_PLACEHOLDER,1)
                if IMAGE_PLACEHOLDER not in content and "image" in content:
                    flags = 2
                    content = content.replace("image", "image:" + IMAGE_PLACEHOLDER,1)
                if len(images) == 1 and IMAGE_PLACEHOLDER not in content and len(content) > 2:
                    flags = 3
            ...
            while IMAGE_PLACEHOLDER in content:
                ...
                if num_image_tokens >= len(image_grid_thw):
                    print(len(images), len(image_grid_thw))
                    print("content:", content)
                    print("images:", len(images),"tokens:", num_image_tokens, "image_grid_thw:", len(image_grid_thw))
                    print("flags:", flags)
